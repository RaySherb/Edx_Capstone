...
In practice the recommendation system would only recommend movies that are predicted to be above the users average rating. This validation set replicates how future users would rate movies, both the movies that were recommended and movies that were not recommended (due to a low score).

As the data set is constantly growing, and the algorithm is being tested in the wild, it is realistic to check the rmse of the model post-implementation. A data pipeline of importing the newest results and adding them to the existing model would also be important. Verifying that the updating rmse is behaving as expected is an easy way of making sure the model has not broken down. 